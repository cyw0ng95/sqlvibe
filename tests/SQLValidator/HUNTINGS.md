...
2. Add a regression test in `tests/Regression/` (or in `validator_test.go`
...
| **Fix** | Added `g.pkOrderBy(tm1, "ASC")` before `LIMIT 10` in both the EXISTS and IN branches of `genSubquery` in `tests/SQLValidator/generator.go`. This forces a deterministic ORDER BY so both databases return the same rows. |
| **Seed** | 1, 2 |
| **Found By** | SQLValidator |
| **Date** | 2026-02-25 |
...
go test -v -run=TestSQLValidator ./tests/SQLValidator/...

# Run with a specific seed for reproduction
go test -v -run=TestSQLValidator_Regression ./tests/SQLValidator/...
```
