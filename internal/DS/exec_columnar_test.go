...
func buildIntCol(vals []int64, nulls []bool) *ColumnVector {
	cv := NewColumnVector("v", TypeInt)
	for i, v := range vals {
		if nulls != nil && i < len(nulls) && nulls[i] {
			cv.AppendNull()
		} else {
			cv.Append(IntValue(v))
		}
	}
	return cv
}

func buildFloatCol(vals []float64, nulls []bool) *ColumnVector {
	cv := NewColumnVector("v", TypeFloat)
	for i, v := range vals {
		if nulls != nil && i < len(nulls) && nulls[i] {
			cv.AppendNull()
		} else {
			cv.Append(FloatValue(v))
		}
	}
	return cv
}

func buildStringCol(vals []string, nulls []bool) *ColumnVector {
	cv := NewColumnVector("v", TypeString)
	for i, v := range vals {
		if nulls != nil && i < len(nulls) && nulls[i] {
			cv.AppendNull()
		} else {
			cv.Append(StringValue(v))
		}
	}
	return cv
}
...
func TestVectorizedFilter_EqualInt(t *testing.T) {
	...
	rb := VectorizedFilter(col, "=", IntValue(2))
	got := rb.ToSlice()
...
func TestVectorizedFilter_LessThan(t *testing.T) {
	...
	rb := VectorizedFilter(col, "<", IntValue(10))
	got := rb.ToSlice()
	if len(got) != 2 || got[0] != 1 || got[1] != 3 {
		t.Fatalf("expected [1 3], got %v", got)
	}
...
func TestVectorizedFilter_GreaterThan(t *testing.T) {
	...
	rb := VectorizedFilter(col, ">", IntValue(4))
	got := rb.ToSlice()
	if len(got) != 2 || got[0] != 1 || got[1] != 3 {
		t.Fatalf("expected [1 3], got %v", got)
	}
...
func TestVectorizedFilter_GreaterEqual(t *testing.T) {
	...
	rb := VectorizedFilter(col, ">=", IntValue(5))
	got := rb.ToSlice()
	if len(got) != 2 || got[0] != 1 || got[1] != 3 {
		t.Fatalf("expected [1 3], got %v", got)
	}
...
func TestVectorizedFilter_SkipsNulls(t *testing.T) {
	...
	rb := VectorizedFilter(col, ">", IntValue(0))
	got := rb.ToSlice()
...
func TestVectorizedFilter_String(t *testing.T) {
	...
	rb := VectorizedFilter(col, "=", StringValue("banana"))
	got := rb.ToSlice()
...
func TestVectorizedFilter_Float(t *testing.T) {
	...
	rb := VectorizedFilter(col, ">", FloatValue(2.0))
	got := rb.ToSlice()
...
func TestColumnarHashJoin_Basic(t *testing.T) {
	left := NewHybridStore([]string{"id", "name"}, []ValueType{TypeInt, TypeString})
	left.Insert([]Value{IntValue(1), StringValue("alice")})
	left.Insert([]Value{IntValue(2), StringValue("bob")})
	left.Insert([]Value{IntValue(3), StringValue("carol")})

	right := NewHybridStore([]string{"uid", "score"}, []ValueType{TypeInt, TypeInt})
	right.Insert([]Value{IntValue(1), IntValue(90)})
	right.Insert([]Value{IntValue(3), IntValue(85)})
...
func TestColumnarHashJoin_NoMatches(t *testing.T) {
	left := NewHybridStore([]string{"id"}, []ValueType{TypeInt})
	left.Insert([]Value{IntValue(1)})
	right := NewHybridStore([]string{"id"}, []ValueType{TypeInt})
	right.Insert([]Value{IntValue(99)})
...
func TestVectorizedGroupBy_Sum(t *testing.T) {
	...
	hs.Insert([]Value{StringValue("A"), IntValue(10)})
	hs.Insert([]Value{StringValue("B"), IntValue(20)})
	hs.Insert([]Value{StringValue("A"), IntValue(5)})
