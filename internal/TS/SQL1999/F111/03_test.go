...
func TestSQL1999_F301_F11103_L1(t *testing.T) {
	sqlvibePath := ":memory:"
	sqlitePath := ":memory:"
	...
	// Test transaction rollback semantics
	tests := []struct {
		name string
		sql  string
	}{
	...
	SQL1999.CompareQueryResults(t, sqlvibeDB, sqliteDB, "SELECT * FROM t1 ORDER BY id", "VerifyRollback")
}
