...
func TestSQL1999_F301_F47101_L1(t *testing.T) {
	sqlvibePath := ":memory:"
	sqlitePath := ":memory:"
	...
	tests := []struct {
		...
		{"ScalarSubqueryInSelect", "SELECT id, (SELECT MAX(val) FROM t1) AS max_val FROM t1 ORDER BY id"},
		{"ScalarSubqueryCount", "SELECT id, (SELECT COUNT(*) FROM t1) AS total FROM t1 ORDER BY id"},
		{"ScalarSubqueryMin", "SELECT id, (SELECT MIN(val) FROM t1) AS min_val FROM t1 ORDER BY id"},
	}
