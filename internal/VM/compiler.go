...
func (c *Compiler) compileColumnRef(col *QP.ColumnRef) int {
	...
	// If column not found AND has table qualifier, this might be an outer reference
...
func (c *Compiler) compileFuncCall(call *QP.FuncCall) int {
	...
	case "NULLIF":
		...
		if len(argRegs) >= 2 {
			cmpReg := c.ra.Alloc()
			c.program.EmitOpWithDst(OpEq, int32(argRegs[0]), int32(argRegs[1]), cmpReg)

			// If cmpReg is 0 or NULL (values are not equal), jump to copy section
			notEqJump := c.program.EmitOp(OpIfNot, int32(cmpReg), 0)

			// Equal: result is NULL
			c.program.EmitLoadConst(dst, nil)

			// Jump to end
			endJump := c.program.EmitOp(OpGoto, 0, 0)
