...
func (bt *BTree) createRoot(key, payload []byte) error {
	...
	bt.writeUint16(page.Data[1:3], uint16(pageSize))
	bt.writeUint16(page.Data[3:5], 0)

	cellOffset := pageSize - len(payload) - len(key) - 2
	bt.writeUint16(page.Data[cellOffset:cellOffset+2], uint16(cellOffset))
	bt.writeUint16(page.Data[cellOffset+2:cellOffset+4], 0)

	n := bt.writeVarint(page.Data[cellOffset+4:], key)
	m := bt.writeVarint(page.Data[cellOffset+4+n:], payload)
	_ = m

	bt.writeUint16(page.Data[1:3], uint16(cellOffset+4+n+m))
...
func (bt *BTree) readKey(page *Page, offset int, isTable bool) []byte {
	if isTable {
		...
		rowID, _ := bt.readVarint(page.Data[offset+n:])
		key := make([]byte, n+len(payloadLen))
		copy(key, payloadLen)
		copy(key[n:], rowID)
		return key
	}
	...
	payloadLen, n := bt.readVarint(page.Data[offset:])
	return page.Data[offset : offset+n+len(payloadLen)]
}

func (bt *BTree) readCellPayload(page *Page, offset int, isTable bool) ([]byte, []byte) {
	if isTable {
		_, n := bt.readVarint(page.Data[offset:])
		rowID, m := bt.readVarint(page.Data[offset+n:])
		key := rowID
		payloadOffset := offset + n + m
		payload := page.Data[payloadOffset:]
		return key, payload
	}

	payloadLen, n := bt.readVarint(page.Data[offset:])
	payloadOffset := offset + n
	payload := page.Data[payloadOffset : payloadOffset+len(payloadLen)]
...
func (bt *BTree) writeCell(page *Page, cell int, key, payload []byte, child uint32) {
	...
	if page.Type == PageTypeLeafTbl {
		cellOffset := int(bt.readUint16(page.Data[1:3])) - len(payload) - len(key) - 2
		cellPointers[cell] = uint16(cellOffset)

		n := bt.writeVarint(page.Data[cellOffset:], key)
		m := bt.writeVarint(page.Data[cellOffset+n:], payload)
		cellOffset += n + m
		bt.writeUint16(page.Data[1:3], uint16(cellOffset))
	} else if page.Type == PageTypeInteriorTbl {
		cellOffset := int(bt.readUint16(page.Data[1:3])) - len(key) - 1 - 8
		cellPointers[cell] = uint16(cellOffset)

		page.Data[cellOffset] = byte(child)
		cellOffset++
		n := bt.writeVarint(page.Data[cellOffset:], key)
		cellOffset += n
		bt.writeUint16(page.Data[1:3], uint16(cellOffset))
...
func (bt *BTree) readChildPage(page *Page, offset int, isTable bool) uint32 {
	if isTable {
		...
		return uint32(bt.bytesToUint64(child))
	}
	return uint32(bt.bytesToUint64(page.Data[offset : offset+4]))
}

func (bt *BTree) readVarint(data []byte) ([]byte, int) {
	var result []byte
...
func (bt *BTree) writeVarint(data []byte, val []byte) int {
	n := len(val)
	copy(data, val)
	return n
}
