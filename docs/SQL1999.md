...
| E071 | Subqueries | 7 | 105 | 60 | 57% | Partial |
...
| F021 | Information Schema | 5 | 70 | 70 | 100% | Complete |
...
- ✅ **E031 Information Schema** - Added `key_column_usage` view, fixed test framework (98% pass rate)
- ✅ **F021 Information Schema** - Fixed test framework to not compare against SQLite (100% pass rate)
- ✅ **E071 Subqueries (Phase 1)** - Implemented IN/NOT IN/EXISTS/NOT EXISTS with subqueries (56% → 57% pass rate)
- ✅ **E071 Subqueries (Phase 2a)** - Fixed scalar subqueries in HAVING clause (E071/01 now 100%)

### High Priority (0% pass - need implementation)
1. **E041 Schema Definition** - CREATE TABLE details incomplete, validation issues
2. **F201 CAST Function** - Type conversion incomplete

### Medium Priority (20-75% pass - need fixes)
1. **E071 Subqueries (Phase 2)** - Remaining gaps:
   - Derived tables (subqueries in FROM clause) - requires parser + VM changes
   - Correlated subqueries - requires VM context passing
   - Scalar subqueries in HAVING clause
2. **E011/E021** - Numeric and character type edge cases
3. **E051/E061** - Query specification and predicates
4. **E091** - Table Expressions (JOINs not fully working)
5. **F031** - Schema manipulation DDL
6. **F041** - Joined tables
7. **F051** - Date and time handling
8. **F261** - CASE expression edge cases

### Detailed E071 Subquery Status (57% complete)
**✅ Implemented**:
- IN/NOT IN with subqueries - fully working (Phase 1)
- EXISTS/NOT EXISTS - fully working (Phase 1)
- Scalar subqueries in SELECT, WHERE, ORDER BY, HAVING - fully working (Phases 1 & 2a)

**⏳ Remaining Gaps**:
- **Derived Tables**: Parser doesn't support subqueries in FROM clause (TableRef structure needs redesign)
- **Correlated Subqueries**: No context passing mechanism in VM for outer row values
- **Nested Subqueries**: Some complex nested patterns fail (3/7 passing in E071/06)
