...
**Passing Test Suites**: 41/56 (73%)
**Failing Test Suites**: 15/56 (27%)

## Recent Updates (2026-02-20)

Major compatibility improvements raising pass rate from 28/56 (50%) to 41/56 (73%):

- **JOIN fixes**: JOIN ON condition fixed (was broken — `FixupWithPos` was overwriting P2 register operand). Used `OpIfNot` approach instead.
- **LEFT JOIN**: Full implementation with `matchReg` per left row; emits null row with NULLs for right columns when no ON match.
- **NATURAL/USING JOIN**: Column deduplication in `SELECT *` output for shared columns.
- **RIGHT JOIN**: Implemented by converting to LEFT JOIN with table swap.
- **VIEW support**: `CREATE VIEW` / `DROP VIEW` / `SELECT FROM view` fully functional.
- **ALTER TABLE**: `ADD COLUMN` and `RENAME TO` operations implemented.
- **INSERT...SELECT**: Full `INSERT INTO t1 SELECT ... FROM t2` syntax supported.
- **Composite PK**: Uniqueness check uses all PK columns as a tuple.
- **CHECK constraints**: Enforced on row insert.
- **INSERT defaults**: Missing columns filled with declared DEFAULT values (not NULL).
- **Table existence check**: `execVMDML` returns `no such table` error properly.
- **Case-insensitive names**: `resolveTableName` helper used throughout; function dispatch via `strings.ToUpper`.
- **Tokenizer**: Keywords normalized to uppercase; doubled `''` quote escape in string literals.
- **Compiler (CG only)**: Duplicate `Compiler` in VM package removed; all compilation in CG.
- **NULL semantics**: `LIKE`/`NOT LIKE`/`NOT IN` return NULL when input is NULL (SQL standard).
- **TYPEOF/RANDOM**: New opcodes `OpTypeof`, `OpRandom` added to VM.
- **ROUND(val, precision)**: Precision argument now passes through to VM.
- **Multi-statement SQL**: `Query()` and `Exec()` now support semicolon-separated multi-statement input.
